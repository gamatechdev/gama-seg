        ...
        `)
            ...
            if (procData) {
                ...
                const supported = procData.filter(p => {
                    // Permite explicitamente a NR 05 caso o id seja 416
                    if (p.id === 416) return true;
                    // Permite explicitamente a NR 06 caso o id seja 417
                    if (p.id === 417) return true;
                    // Permite explicitamente a NR 17 caso o id seja 428
                    if (p.id === 428) return true;
                    // Permite explicitamente a NR 34 caso o id seja 445
                    if (p.id === 445) return true;

                    // Remove todos os assentos e passa para minúsculo
                    const lowerName = p.nome.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");

                    // Retorna verdadeiro para que o nome com NR05 não seja excluído
                    if (lowerName.includes('nr05') || lowerName.includes('nr 05')) return true;
                    // Retorna verdadeiro para que o nome com NR10 não seja excluído
                    if (lowerName.includes('nr10') || lowerName.includes('nr 10')) return true;
                    // Retorna verdadeiro para que o nome com NR32 não seja excluído
                    if (lowerName.includes('nr32') || lowerName.includes('nr 32')) return true;
                    // Retorna verdadeiro para que o nome com NR33 não seja excluído
                    if (lowerName.includes('nr33') || lowerName.includes('nr 33')) return true;

                    // Bloqueia qualquer valor que não tenha idcategoria de treinamento (46)
                    if (p.idcategoria !== 46) return false;
                    // Usa tags mapeadas detectadas do nome
